...
for k in range(len(fnames_m)):
    ...
    if Analisis_de_Fourier == 1: 
        ...
        #28 Mar 25 Agrego taus calculados con armonicos de orden superior 
        taus=[]
        for n in range(3):
            x=np.tan((2*n+1)*espectro_ref[2]-espectro_f_amp_fase_m[2][n])/(2*np.pi*espectro_f_amp_fase_m[0][0])
            taus.append(x)
            print(n,f'frec: {espectro_f_amp_fase_m[0][n]:9.0f} Hz -',f'tau_{n} = {x:6.3f} s')
        taus=np.array(taus)
...
Taus_ord_sup=[Taus_ord_sup[i]*1e9 for i in indices_to_stay] #paso a ns
...
#%% guardo Temp y taus de orden superior :| T |tau1|tau3|tau5|tau7|
identif= np.array([int(fn.split('_')[-1][4:-4]) for fn in fnames_m])
data=np.column_stack([identif,Taus_ord_sup])
np.savetxt(os.path.join(output_dir,'taus_ord_sup.txt'), data,header='| fn | tau1 | tau3 | tau5 |' ,fmt=('%3.0f','%7.2f','%7.2f','%7.2f'))
...
# ax[1].plot(t,M)
# ax[1].set_xlabel('t')
# ax[0].set_title('H',loc='left')
# ax[1].set_title('M',loc='left')
# for a in ax:
#     a.grid()
# plt.xlim(0,t[-1])
...
# for a in ax:
#     a.grid()
#     a.axvspan(0,1/frec, facecolor='#2ca02c',label='Período 1/{N_ciclos}',alpha=0.4)
# plt.xlim(0,t[-1])
...
#                                                                                                         filtro=0.05,frec_limite=20*frec,
#                                                                                                         name='test')
...
# #%% TAU a partir de la FASE
# tau=np.tan(espectro_f_amp_fase_m[2][0])/(2*np.pi*espectro_f_amp_fase_m[0][0])
# print('tau=',tau)

# frecs=espectro_f_amp_fase_m[0]
# Fase= espectro_f_amp_fase_m[2]
# fig,ax=plt.subplots(constrained_layout=True)

# ax.plot(frecs,Fase,'.-',label='Phi')
# ax.plot(frecs,np.tan(Fase),'o-',label='Tan(phi)')
...
# # %% Idem, pero con el ultimo periodo

# archivo = '100kHz_datos_recortado.txt'
# # Leer el archivo y cargar los datos en un array de NumPy
# data = np.loadtxt(archivo)
...
#                                                                                                         filtro=0.05,frec_limite=20*frec,
#                                                                                                         name='test')
# # fig_fourier_rec.savefig(os.path.join(os.getcwd(),'simulado_Espectro_rec.png'),dpi=200,facecolor='w')
# # fig2_fourier_rec.savefig(os.path.join(os.getcwd(),'simulado_Rec_impar_rec.png'),dpi=200,facecolor='w')

# # Guardar espectro en .txt
# with open(os.path.join(os.getcwd(),'simulado_Espectro_rec.txt'), 'w') as f:
#     f.write(f'# {datetime.now().strftime("%y%m%d_%H%M%S")}\n')
...
# ax.grid()
# ax.set_xlabel('frec')
# plt.savefig('fase_simulado_rec.png')

# # %%
# np.tan(espectro_ref[2]-espectro_f_amp_fase_m[2][0])/(2*np.pi*espectro_f_amp_fase_m[0][0])
# %%
